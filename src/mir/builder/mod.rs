    ...
    Db: DbGet<TypeCheck> + DbGet<GetItem> + DbGet<GetItemRaw> + DbGet<ExportedTypes> + DbGet<AutoDrop>,
{
    ...
    fn is_shared_copy_impl(&self, name_id: NameId) -> bool {
        let typ = self.types.get_generalized(name_id).ignore_forall().follow(&self.types.bindings);
    ...
    fn define_release_function(&mut self, type_definition: &cst::TypeDefinition) {
        ...
        // `fn(Pointer, <caps>) -> Unit`; genericity lives in `generic_count` + the body's
        // Instantiates.
        let mut parameters = vec![Type::POINTER];
        parameters.extend(drop_cap_types.iter().cloned());
    ...
    fn define_shared_drop_impl(&mut self, type_definition: &cst::TypeDefinition) {
        ...
        let method_id = self.new_definition(method_name.clone(), None, generic_count, function_type.clone(), |this| {
            ...
            let cap_values = if caps.is_empty() {
                ...
                this.unpack_closure_environment(caps.iter().copied(), environment);
    ...
    /// (instantiating the generic release function when the type is generic).
    ///
    ...
    fn emit_release_call(
        ...
        let mut parameters = vec![Type::POINTER];
        parameters.extend(drop_caps.iter().map(|cap| self.type_of_value(cap)));
        let fn_type = Type::Function(Arc::new(crate::mir::FunctionType {
            parameters,
            environment: Type::NO_CLOSURE_ENV,
            return_type: Type::UNIT,
