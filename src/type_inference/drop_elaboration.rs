...
impl TypeChecker<'_, '_> {
    ...
    pub(super) fn assignment_overwrite_drop(&mut self, assignment: &cst::Assignment, id: ExprId) {
        ...
        // reference: dropping the old value there is a through-reference drop (excluded here,
        // and the capture set is not even known until the lambda ends). Locals only.
    ...
    fn param_escapes_in_body(&self, expr: ExprId, name: NameId) -> bool {
        ...
        match self.resolved_expr(expr).as_ref() {
    ...
    pub(super) fn reference_target_is_function_local(&self, target: ExprId) -> bool {
        match self.resolved_expr(target).as_ref() {
    ...
    fn type_needs_no_drop_inner(&mut self, typ: &Type, depth: u32) -> bool {
        ...
        match &typ {
            ...
            // An effect row is evidence: borrowed data the compiler threads through calls,
            // owned by nobody and never torn down at a scope exit.
            Type::Effects(..) => true,
