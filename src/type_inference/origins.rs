...
//! propagation through `r2 = r` is a table lookup), two consumers: task 10's escape check (a tail
//! `Local` origin is a dangling return) and, later, the last-use / release-placement stage.
//!
...
//! uses).
//!
//! `OriginSet` is a `Vec` rather than the design's `SmallVec<[_; 2]>` (no smallvec dep in-tree); the
...
/// Where a reference-carrying value's references may point (the design's `Origin`).
#[derive(Clone, Debug)]
#[allow(dead_code, reason = "`Param`'s NameId and `Immortal` are consumed by 09/10")]
...
impl TypeChecker<'_, '_> {
            ...
            cst::Expr::Variable(path) => match self.path_origin(*path) {
                Some(NameOrigin::Local(name)) => {
                    if let Some(recorded) = bindings.get(&name) {
                        recorded.clone() // propagation through bindings -- the 1f hole
    ...
    /// does not read as containing a reference. Otherwise, unknown callee: the 1f heuristic -- the
    ...
    /// `offset` / `ptr_to_mut`); the shallow `type_contains_reference` gate -- shared with 1f and
    ...
    fn lambda_origin(&self, lambda_expr: ExprId, bindings: &mut FxHashMap<NameId, OriginSet>) -> OriginSet {
        ...
        for name in locals {
            ...
            if let Some(recorded) = bindings.get(&name) {
                ...
                // A non-`move` closure borrows its owned captures (10 turns these into checked
                // borrows). Reference-typed captures point elsewhere; `move` captures are owned by
                // the env (03/04) -- neither is a borrow of this function's storage.
                set.push(Origin {
