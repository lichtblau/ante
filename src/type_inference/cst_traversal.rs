...
impl<'local, 'inner> TypeChecker<'local, 'inner> {
    pub(super) fn check_definition(&mut self, definition: &Definition, is_top_level: bool) {
        ...
        if is_top_level
            && let Expr::Lambda(lambda) = self.resolved_expr(definition.rhs).as_ref()
        {
        ...
        let actual = match self.path_origin(path) {
            ...
            Some(Origin::Local(name)) => {
                ...
                if !self.suppress_move_record && !borrowed_as_callee {
                    ...
                    } else if self.drop_elaboration_active() && !self.type_needs_no_drop(&typ) {
                        // The other half of the same decision: this read copies the value and leaves
                        // the source's drop where it was, so the copy owes its own reference to
                        // whatever refcounted data it carries (`copy_place_paths`).
                        self.current_extended_context_mut().mark_copy_place_path(path);
                    }
                }
                typ
            },
    ...
    fn eta_expand_borrowing_value(&mut self, id: ExprId, typ: Type, expected: &Type) -> Type {
        ...
        let wrapper = if function_type.parameters.iter().all(|param| !param.is_implicit) {
            ...
            let bare = Arc::new(FunctionType {
                ...
                // Same function, only without its implicit parameters: it performs exactly what
                // the wrapped function performs, so the row is copied rather than assumed pure.
                effects: function_type.effects.clone(),
            });
    ...
    fn infer_call(&mut self, call: &cst::Call, expected: &Type, call_expr: ExprId) -> Type {
        ...
        let callee_is_variable = matches!(self.resolved_expr(call.function).as_ref(), Expr::Variable(_));
    ...
    fn infer_lambda_impl(
        ...
        if let Some(outer_names) = outer_names {
            ...
            if !options.handler_scoped {
                ...
                // builder).
                if !super::free_variables::is_pointer(&function_type.environment, &self.bindings) {
    ...
    fn infer_member_access(&mut self, member_access: &cst::MemberAccess, expected: &Type, expr: ExprId) -> Type {
        ...
        if let Some((field, field_index)) = fields.get(&member_access.member) {
            ...
            if !struct_is_indirect && let Some(parent_path) = self.try_build_move_path(member_access.object) {
                ...
                if !old_suppress_record {
                    if !self.type_is_copy(&field) {
                        let location = expr.locate(self);
                        self.move_tracker.record_move(move_path, location);
                    } else if self.drop_elaboration_active() && !self.type_needs_no_drop(&field) {
                        // A copied field read: same reasoning as a copied variable read
                        // (`copy_place_exprs`). A field behind a `ref`/`Ptr` never reaches here --
                        // it is coerced into a `Copy` `(.*)` call, which retains on its own.
                        self.current_extended_context_mut().mark_copy_place_expr(expr);
                    }
                }
            }
